...
class ContentController extends BaseController
{
    ...
    public function createAction(Request $request, ContentAdmin $admin)
    {
        ...
        if ($request->isMethod('POST')) {
            try {
                ...
                $this->setFlash('success', $this->get('translator')->trans('content.create.flash.success'), array('%type%' => $this->get('translator')->transChoice($admin->getOption('label'), 1), '%name%' => $admin->getEntityName($entity)));
    ...
    public function updateAction(Request $request, ContentAdmin $admin)
    {
        ...
        if ($request->isMethod('POST')) {
            try {
                $this->save($admin, $form, $entity);
                $this->setFlash('success', $this->get('translator')->trans('content.update.flash.success'), array('%type%' => $this->get('translator')->transChoice($admin->getOption('label'), 1), '%name%' => $admin->getEntityName($entity)));
                $redirectUrl = $this->getRequest()->get('saveMode') === ContentAdmin::SAVEMODE_CONTINUE ?
                    $this->getRoutingHelper()->generateUrl($admin, 'update', array('id' => $entity->getId())) :
                    $this->getRoutingHelper()->generateUrl($admin, 'index');
    ...
    public function modalDelete(Request $request, ContentAdmin $admin, $entity)
    {
        if($request->isMethod('post')) {
            ...
            $this->setFlash('success', $this->get('translator')->trans('content.delete.flash.success'), array('%type%' => $this->get('translator')->transChoice($admin->getOption('label'), 1), '%name%' => $admin->getEntityName($entity)));
            $result = array(
                'entity_id' => $entity->getId(),
                'entity_name' => $admin->getEntityName($entity)
            );
